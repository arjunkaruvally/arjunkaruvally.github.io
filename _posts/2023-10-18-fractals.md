...
We know a line is 1D, and a filled cirlce is 2D, so let's start with a reasonable definition of dimension.

>**_NOTE:_**  The note content.

That must mean that we can represent it in a 1D space. So let's take a circle of radius 1 without any area, and only the circumferece.
We need a 2D plane to draw it. But, here is the problem - although we use a 2D plane to represent the circle, you dont really need 2 numbers to specify
all of it's points. 
That is, you can take any $x$ and solve the equation $y = \sqrt{1-x^2}$ to get y. So, somehow your $x$ uniquely specified 
all your points in the circle, which must mean that the circle we have is actually a 1D object even through we draw it in 2D.
Another way to look at this is that you can construct a coordinate system which is not a plane like in the typical coordinate system
and obtain 
