...
In my class, "[Nature Inspired Design in Computing](https://arjunkaruvally.github.io/FYS-NID/)," we recently delved into the intriguing world of fractals. To share these insights more broadly, I've put together this blog post. We'll start by unraveling the foundational definitions and insights, then progress to the captivating mathematics of fractals, highlighting how our standard understanding of spatial dimensions might not quite grasp their unique nature. By the end, we'll introduce the concept of the box counting dimension—often referred to as the [Minkowski-Bouligand dimension](https://en.wikipedia.org/wiki/Minkowski%E2%80%93Bouligand_dimension)—illustrating that fractals interestingly possess fractional dimensions!
...
**Attempts to Formally Define Fractals and Their Shortcomings with Examples**
...
3. **Recursive Definition**: 
   - **Definition**: Fractals can be produced by repeating a process.
...
We will consider the Serpienski triangle to tease out the properties of fractals. I hope by the end of this section, there will be sufficient evidence to convince that conventional integer dimensional spaces are inadequate to understand them.
...
What you're left with is a pattern that, no matter how closely you look at it, keeps revealing smaller triangles. self similarity! The Sierpinski triangle may look complicated, but its construction is based on a straightforward and repetitive process. It's a perfect example of how simple rules can lead to complex and beautiful patterns.
...
To understand why introducing a fractional dimension is even necessary for fractals, lets see how much our conventional notions of integer-dimensional spaces work with the Serpienski triangle. What we mean by "conventional" is that our notion that lines are 1D objects, areas like triangle and rectangle are 2D objects, volumes like sphere and cube are 3D objects and so on. We will use this temporary notion of dimension for now, and formalize it later.

**Perimeter**

Computing the perimeter of an infinite object like the Sierpinski triangle is challenging. One way to approach this problem is by first figuring our how the perimeter *changes* when you repeat the process of generating a fractal.
Once that is done, we can generalize it to infinity.

We begin with a full equilateral triangle with area shaded in black. Assume each side has a length of $d$. The perimeter of this triangle is $3d$. Now, we can start our Sierpinski procedure.

**Iteration 1**

Connect the midpoints of the big triangle and remove the resulting middle triangle. The figure we get now is
...
function annotate_triangle(context, h, w, steps){
    ...
    var top = h/2;
	var left = w/4;
	recursive_annotate(context,h/2,w/2,top,left, steps, max_steps);
}
...
function do_annotation(context,xa,ya,xb,yb,xc,yc,color, scale=1, step=1) {
    var fontsize = Math.floor(50*scale)
    ...
    if(step > 1){
        context.fillText(`d/${step}`, ann_x, ann_y);
    }
    else{
